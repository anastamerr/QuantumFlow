...
`
  ...
  for (let i = 0; i < qubits.length; i++) {
    ...
    // Qubit label
    svg += `<rect x="${padding}" y="${y - cellHeight/2}" width="${labelWidth}" height="${cellHeight}" fill="#EBF8FF" stroke="#BEE3F8" />
`
...
`
      ...
      // Draw Toffoli gate
      const control1 = gate.controls?.[0] || 0
      const control2 = gate.controls?.[1] || 1
      const target = gate.targets?.[0] || 2
...
`
      ...
      // Draw standard gate
      svg += `<rect x="${x - 20}" y="${y - 20}" width="40" height="40" rx="5" fill="${getGateColor(gateColor)}" class="gate-rect" />
`
...
`
      ...
      if (gateDefinition.params && gateDefinition.params.length > 0 && gate.params) {
        ...
        const paramValue = gate.params[paramName] || gateDefinition.params[0].default
