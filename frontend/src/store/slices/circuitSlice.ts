...
export const circuitSlice = createSlice({
  ...
  reducers: {
    ...
    addGate: (state, action: PayloadAction<Omit<Gate, "id">>) => {
      ...
      // Update maxPosition if needed (extract numeric position from object or use directly)
      const numericPosition =
        typeof newGate.position === "number"
          ? newGate.position
          : newGate.position?.x ?? 0;
      if (numericPosition >= state.maxPosition) {
        state.maxPosition = numericPosition + 5; // Add some buffer
      }
    },
    updateGate: (
      state,
      action: PayloadAction<{ id: string; updates: Partial<Gate> }>
    ) => {
...
export const selectCircuitDescription = (state: RootState) =>
  state.circuit.description;
export const selectMaxPosition = (state: RootState) =>
  state.circuit.maxPosition;
