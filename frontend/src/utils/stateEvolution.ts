...
export const applyTwoQubitGate = (
  ...
  const controlQubit =
    gate.qubit !== undefined
      ? gate.qubit
      : (gate.controls && gate.controls[0]) || 0;
...
export const simulateGateApplication = (
  ...
  if (
    ["h", "x", "y", "z", "s", "t", "rx", "ry", "rz", "p"].includes(gate.type)
  ) {
...
const applyHadamard = (
  ...
  addToState(newState, basisState, [
    amplitude[0] * factor,
    amplitude[1] * factor,
  ]);
  addToState(newState, otherState, [
    amplitude[0] * factor * sign,
    amplitude[1] * factor * sign,
  ]);
};
...
const applyRotationX = (
  ...
  if (targetBit === "0") {
    ...
    const newAmplitude0: Complex = [amplitude[0] * cos, amplitude[1] * cos];
    ...
    const newAmplitude1: Complex = [amplitude[0] * cos, amplitude[1] * cos];
...
const applyRotationY = (
  ...
  if (targetBit === "0") {
    ...
    const newAmplitude0: Complex = [amplitude[0] * cos, amplitude[1] * cos];
    const newAmplitude1: Complex = [-amplitude[0] * sin, -amplitude[1] * sin];

    addToState(newState, basisState, newAmplitude0);
    addToState(newState, otherState, newAmplitude1);
  } else {
    // Original state |1⟩ contributes sin(θ/2) to |0⟩ and cos(θ/2) to |1⟩
    const newAmplitude0: Complex = [amplitude[0] * sin, amplitude[1] * sin];
    const newAmplitude1: Complex = [amplitude[0] * cos, amplitude[1] * cos];
