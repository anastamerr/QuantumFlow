...
import {
  prepareGatesForCodeGeneration,
  validateCircuitInput,
  groupGatesByPosition,
...
export const generateCirqCode = (
  ...
  const cirqImports = ["import cirq", "import numpy as np", ...imports];

  if (optimize) {
    cirqImports.push(
      "from cirq.optimizers import EjectZ, EjectPhasedPaulis, DropEmptyMoments, DropNegligible"
    );

    if (optimizationOptions.enableAdvancedOptimization) {
      cirqImports.push(
        "from cirq.optimizers import MergeInteractions, MergeSingleQubitGates"
      );
      cirqImports.push(
        "from cirq.transformers import optimize_for_target_gateset, drop_empty_moments, drop_negligible_operations"
      );
      ...
      switch (gate.type) {
        ...
        case "rx":
          ...
          if (isNaN(theta)) {
            ...
            const rxAngle =
              Math.abs(theta) <= 2 * Math.PI ? theta : (theta * Math.PI) / 180;
        ...
        case "rz":
          ...
          const rzPhi = Number(
            gate.params?.phi || gate.params?.theta || gate.params?.angle || 0
          );
          if (isNaN(rzPhi)) {
            ...
            const rzAngle =
              Math.abs(rzPhi) <= 2 * Math.PI ? rzPhi : (rzPhi * Math.PI) / 180;
        ...
        case "p":
          ...
          if (isNaN(phasePhi)) {
            ...
            // Check if value is already in radians or degrees
            const phaseAngle =
              Math.abs(phasePhi) <= 2 * Math.PI
                ? phasePhi
                : (phasePhi * Math.PI) / 180;
        ...
        case "cp":
          ...
          let cpAngle = 0;
          if (isNaN(cpPhi)) {
            gateSection += `# Warning: Invalid parameter for CP gate, using 0\n`;
          } else {
            cpAngle =
              Math.abs(cpPhi) <= 2 * Math.PI ? cpPhi : (cpPhi * Math.PI) / 180;
          }
          ...
          if (
            ...
            gateSection += `circuit.append(cirq.cphase(${cpAngle})(qubits[${gate.controls[0]}], qubits[${gate.targets[0]}]))\n`;
          } else if (
            gate.qubit !== undefined &&
            gate.targets &&
            gate.targets.length > 0
          ) {
            gateSection += `circuit.append(cirq.cphase(${cpAngle})(qubits[${gate.qubit}], qubits[${gate.targets[0]}]))\n`;
          } else {
            gateSection += `# Warning: CP gate missing control or target qubit\n`;
          }
          break;
        case "swap":
...
function generateOptimizationSection(options: OptimizationOptions): string {
  ...
  if (options.enableAdvancedOptimization && options.advancedOptions) {
    ...
    if (options.advancedOptions.synthesisLevel > 0) {
      ...
      optimizationSection +=
        "from cirq.transformers import optimize_for_target_gateset, merge_single_qubit_gates_into_phased_x_z\n";
      optimizationSection +=
        "circuit = optimize_for_target_gateset(circuit, gateset=cirq.SqrtIswapTargetGateset())\n";
      optimizationSection +=
        "circuit = merge_single_qubit_gates_into_phased_x_z(circuit)\n";
    ...
    if (options.advancedOptions.depthReduction) {
      ...
      optimizationSection +=
        "# Note: Manual optimization may be needed for depth reduction in current Cirq versions\n";

      if (
        options.advancedOptions.maxDepth &&
        options.advancedOptions.maxDepth > 0
      ) {
    ...
    if (options.advancedOptions.qubitMapping) {
      ...
      if (hardwareModels[hardwareModel]) {
        ...
        if (hardwareModel === "linear") {
          ...
          optimizationSection +=
            "    device_graph.add_edge(qubits[i], qubits[i + 1])\n";
        } else if (hardwareModel === "grid") {
          optimizationSection += "# Grid topology\n";
          optimizationSection += "import math\n";
          optimizationSection +=
            "grid_size = math.ceil(math.sqrt(len(qubits)))\n";
          optimizationSection += "for i in range(len(qubits)):\n";
          optimizationSection +=
            "    row, col = i // grid_size, i % grid_size\n";
          optimizationSection +=
            "    if col < grid_size - 1 and i + 1 < len(qubits):\n";
          optimizationSection +=
            "        device_graph.add_edge(qubits[i], qubits[i + 1])\n";
          optimizationSection +=
            "    if row < grid_size - 1 and i + grid_size < len(qubits):\n";
          optimizationSection +=
            "        device_graph.add_edge(qubits[i], qubits[i + grid_size])\n";
        } else {
          optimizationSection += "# Custom device topology\n";
          optimizationSection += "for i in range(len(qubits)):\n";
          optimizationSection += "    for j in range(i + 1, len(qubits)):\n";
          optimizationSection +=
            "        device_graph.add_edge(qubits[i], qubits[j])\n";
        }

        // Map the circuit to the device
        optimizationSection += "\n# Map circuit to device topology\n";
        optimizationSection +=
          "mapped_circuit = route_circuit(circuit, device_graph";
    ...
    if (options.advancedOptions.noiseAware) {
      ...
      optimizationSection +=
        "# Note: In a real implementation, you would customize this based on hardware noise characteristics\n";
      optimizationSection +=
        "from cirq.contrib.noise_models import DepolarizingNoiseModel\n";
      optimizationSection +=
        "noise_model = DepolarizingNoiseModel(depolarizing_probability=0.001)\n";
      optimizationSection +=
        "noisy_circuit = cirq.Circuit(noise_model.noisy_moments(circuit.moments, system_qubits=qubits))\n";

      // Comment on the noisy circuit, but keep using the optimized one
      optimizationSection +=
        "# Created a noisy circuit model for simulation, but continuing with the optimized circuit\n";
    }

    // Print circuit statistics
    optimizationSection += "\n# Print circuit statistics\n";
    optimizationSection +=
      'print(f"Original circuit length: {len(circuit.moments)}")\n';
    optimizationSection +=
      'print(f"Original circuit operations: {sum(len(moment) for moment in circuit.moments)}")\n';
    optimizationSection +=
      'print(f"Optimized circuit length: {len(circuit.moments)}")\n';
    optimizationSection +=
      'print(f"Optimized circuit operations: {sum(len(moment) for moment in circuit.moments)}")\n';
  }
