...
const GateParamsPanel = () => {
  ...
  const updateGateWithDebounce = useCallback(
    (updates: any) => {
      if (!validateGateConfiguration()) {
        ...
        toast({
          ...
          description:
            validationError || "Target and control qubits must be different",
          status: "error",
          duration: 3000,
          isClosable: true,
        });
                  ...
                  {param.type === "number" && (
                    <NumberInput
                      ...
                      onChange={(_, valueAsNumber) =>
                        handleParamChange(param.name, valueAsNumber)
                      }
            ...
            <Box mt={4}>
              ...
              {gateDefinition.targets && gateDefinition.targets > 0 && (
                <VStack align="stretch" spacing={2} mt={2}>
                  ...
                  {Array.from({ length: gateDefinition.targets }).map(
                    (_, index) => {
                      ...
                      if (availableQubits.length === 0) {
                        return (
                          ...
                          >
                            No available target qubits. Add more qubits or
                        ...
                        <Select
                          ...
                          value={
                            targets[index] !== undefined
                              ? targets[index]
                              : availableQubits[0]?.id || 0
                          }
                          onChange={(e) =>
                            handleTargetChange(index, parseInt(e.target.value))
                          }
                          isDisabled={
                            isUpdating || availableQubits.length === 0
                          }
                          isInvalid={
                            validationError !== null &&
                            validationError.includes("target")
                          }
                        >
                ...
                <VStack align="stretch" spacing={2} mt={2}>
                  ...
                  >
                    <option value={selectedGate.qubit}>
                      {qubits.find((q) => q.id === selectedGate.qubit)?.name ||
                        `q${selectedGate.qubit}`}
                    </option>
                  </Select>
                  <Text fontSize="xs" color={infoTextColor}>
                    The control qubit is fixed based on where you placed the
                    gate
                  </Text>
                </VStack>
              )}
              ...
              {!isCNOTorCZ &&
                ...
                gateDefinition.controls > 0 && (
                  <VStack align="stretch" spacing={2} mt={2}>
                    ...
                    {Array.from({ length: gateDefinition.controls }).map(
                      (_, index) => {
                        ...
                        if (availableQubits.length === 0) {
                          return (
                            <Text
                              key={`control-empty-${index}`}
                              fontSize="xs"
                              color={errorTextColor}
                            >
                              No available control qubits. Add more qubits or
                        ...
                        return (
                          <Select
                            ...
                            isInvalid={
                              validationError !== null &&
                              validationError.includes("control")
                            }
                          >
                            {availableQubits.map((qubit) => (
                              <option key={qubit.id} value={qubit.id}>
              ...
              {(targets.length > 0 || controls.length > 0) && (
                <Text fontSize="xs" color={infoTextColor} mt={2}>
                  Target and control qubits must be different for proper quantum
                  operations
                </Text>
              )}
            </Box>
