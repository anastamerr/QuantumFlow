    ...
    (item: DroppedGate, position: CircuitPosition): void => {
      try {
        ...
        if (customGate && isCustomGate(customGate)) {
          ...
          const expandedGates = expandCustomGate(
            customGate,
            position.qubit,
            position.position
          );

          // Add all expanded gates to the circuit (they already have required qubit and position)
          expandedGates.forEach((gate) => {
            dispatch(addGate(gate as any));
          });
