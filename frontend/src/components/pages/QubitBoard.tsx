...
import { QubitPosition, Player } from '@/types/qubitTouchdown' 

const NODE_POSITIONS: Record<string, { top: string; left: string }> = {
  '+': { top: '5%', left: '50%' }, '0': { top: '30%', left: '25%' }, '-i': { top: '30%', left: '75%' },
  '+i': { top: '60%', left: '25%' }, '1': { top: '60%', left: '75%' }, '-': { top: '85%', left: '50%' },
}
...
function BoardNode({ pos, isBallHere, isGoal }: { pos: string; isBallHere: boolean; isGoal: boolean }) {
  const node = NODE_POSITIONS[pos];
  
  // Chakra Color Props
  let bg = isBallHere ? 'yellow.400' : (isGoal ? 'blue.300' : 'yellow.200');
  let border = isBallHere ? 'orange.500' : (isGoal ? 'blue.600' : 'yellow.600');
  let shadow = isBallHere ? '0 0 20px rgba(255, 200, 0, 0.9)' : 'none';
  // Custom ring style for goal
  const ringStyle = isGoal && !isBallHere ? { boxShadow: '0 0 0 4px #3182ce, 0 0 0 6px #22543d' } : {};

  return (
    <Box position="absolute" top={node.top} left={node.left} transform="translate(-50%, -50%)" zIndex={2}>
      <Box
        w="80px" h="80px" borderRadius="full"
        bg={bg} color="black"
        display="flex" alignItems="center" justifyContent="center"
        borderWidth={3} borderColor={border}
        fontWeight="black" fontSize="2xl"
        boxShadow={shadow}
...
export const QubitBoard = ({ ballPosition, players }: { ballPosition: string, players: Player[] }) => {
  ...
  return (
    <Box flex="1" w="full" h="full" position="relative" borderRadius="md" borderWidth={1} borderColor="whiteAlpha.500" overflow="hidden" bg="green.800">
        {EDGES.map((edge, idx) => <EdgeArrow key={idx} edge={edge} />)}
        {Object.keys(NODE_POSITIONS).map((pos) => (
            <BoardNode 
                key={pos} 
                pos={pos} 
                isBallHere={ballPosition === pos} 
                isGoal={pos === p1Goal || pos === p2Goal} 
            />
        ))}
    </Box>
  )
